from utils import verificar_tentativas_restantes, validar_posicao, tratar_posicao
from words import remover_acentos, finalizar_coletar_palavras, verificar_palavra_apenas_letras, verificar_palavra_ja_existe, verificar_tamanho_palavra, registrar_palavras
...
def pedir_posicao(tamanho_matriz):
    ...
    for indice in range(0, 2): # pedindo as posições da palavra ao usuário
        while(verificar_tentativas_restantes(tentativas)): # enquanto a posição for inválida
            ...
            if(finalizar_coletar_palavras(posicao)): # verificando se o usuário deseja encerrar a procura de palavras
